...
impl<'a> Parser<'a> {
    ...
    /// identifier names.
    fn parse_arg_general(&mut self, require_name: bool, is_trait_item: bool, allow_variadic: bool) -> PResult<'a, Arg> {
        ...
        let is_variadic = |p: &mut Parser| {
            if allow_variadic && p.token == token::DotDotDot {
                p.bump();
                true
            } else {
                false
            ...
            self.parse_seq_to_before_end(
                ...
                |p| {
                    if p.token == token::DotDotDot {
                        ...
                        match p.parse_arg_general(named_args, false, allow_variadic) {
                            Ok(arg) => {
                                if arg.variadic {
                                    variadic = true;
                                    if p.token != token::CloseDelim(token::Paren) {
